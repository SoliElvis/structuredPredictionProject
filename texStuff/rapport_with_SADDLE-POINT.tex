...
\\
In this paper, we synthesize the recent advances in Frank-Wolfe methods and discuss the contexts in which each variant is appropriate. Starting from the classical F-W, we motivate each variant with the problems it addresses and we discuss the pitfalls and the intrinsic trade-offs. Moreover, in the saddle-point context, we compare an earlier algorithm, namely the Dual Extragradient, with a Saddle-point variant of the Frank-Wolfe algorithm.
Finally we evaluate the performance of each with text data in the context of word alignment to see how reasonable are the assumptions (providing theoretical guarantees), and to get an idea whether each trade-off is worth it.
\end{abstract} 
%%%%%%%%%%%%%%%%%       FRANK WOLFE
