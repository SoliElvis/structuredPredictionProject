...
It used to be the typical approach to solving a problem that is in
the minimax formulation to dualize and then solve the resulting convex
minimization problem through a generic non linear solver.
In \citet{taskarStructuredPredictionDual2006} it is proposed to use
first-order algorithms designed for saddle point problems. 
This enables us to consider a richer class of problems that we can tackle,
especially for problems where dualizing yield a QP that is not efficiently
solvable. We first take a stab at the saddle-point formulation:
\begin{equation}
